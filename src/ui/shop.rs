...
use ratatui::{style::{Style, Stylize}, widgets::{Block, Row, Table, TableState}};
...
impl ShopPage {
    pub fn new(shop: Rc<RefCell<Shop>>) -> Self {
        Self {
            inventory_table_state: TableState::default().with_selected(if shop.borrow().get_inventory().len() > 0 {
                Some(0)
            } else {
                None
            }),
...
impl RenderablePage for ShopPage {
    ...
    fn handle_and_transact(&mut self, event: ratatui::crossterm::event::Event) -> Option<ratatui::crossterm::event::Event> {
        Some(event)
    }
}
